...
import snowmada.main.view.HomeView.GalleryUploadTask;
import snowmada.main.view.HomeView.ImageUploadTask;
...
public class ProfileEdit extends BaseView{
	...
	age = bundle.getString("age");
       ...
       et_fname.setText(myApp.getAppInfo().userFirstName);
       et_lname = (EditText) findViewById(R.id.et_lname);
       et_lname.setText(myApp.getAppInfo().userLastName);
	...
	Log.e("TAG12", myApp.getAppInfo().image);
	imageLoader.DisplayImage(myApp.getAppInfo().image, iv_user_image);
	...
	btn_submit.setOnClickListener(new OnClickListener() {
	    ...
	    @Override
	    public void onClick(View v) {
		System.out.println("!-- Shred style "+mountain[Spinner_pos]);
		if(isValid())
			...
			protected String doInBackground(String... param) {
			   	try {
					...
					HttpContext localContext = new BasicHttpContext();					
						HttpPost httpPost = new HttpPost(URL.PROFILE_IMAGE_UPDATE.getUrl());
			...
			if(sResponse!=null){
				...
				try {
					...
					if(obj.getBoolean("status")){
					     filepath = null;
					        if(!obj.isNull("image")){
					            Global.profileImageeditflag = true;
	     ...
	     String about_me ;
			...
			protected Boolean doInBackground(String... param) {
				...
				try {
					...
					JSONObject res = KlHttpClient.SendHttpPost(URL.PROFILE_INFO_UPDATE.getUrl(), req);
			...
			protected void onPostExecute(Boolean sResponse) {
			    dismissProgressDialog();
			if(sResponse){					
					    Global.user_location = loc;
					    Global.user_fav_mountain = favorite_mountain;
					    Global.user_shred_style = shred_style;
					    Global.user_about_me = about_me;
					    Global.profileeditflag= true;
					    Global.user_age = et_age.getText().toString().trim();
					    myApp.getAppInfo().setFirstname(et_fname.getText().toString().trim()); 
					    myApp.getAppInfo().setLasttname(et_lname.getText().toString().trim());
